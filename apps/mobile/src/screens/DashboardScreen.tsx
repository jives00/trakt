...
import { View, Text, ScrollView, FlatList, TouchableOpacity, ImageBackground, StyleSheet, ActivityIndicator, Dimensions } from "react-native";
...
import type { UpNextItem, ScheduleItem, DashboardStats, RecentItem, RecommendationItem, StatsAllTime, NowPlayingItem } from "@trakt/types";
...
export default function DashboardScreen() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([up, sched, stats, recent, at, srecs, mrecs, art]) => {
  ...
  return (
    <ScrollView style={s.root} contentContainerStyle={s.content}>
      {nowPlaying ? (
        ...
        <HeroSection alltime={alltime} heroArt={heroArt} />
      )}
      ...
      {upNext.length > 0 && (
        <Section title="Up Next">
          <FlatList
            horizontal
            data={upNext}
            keyExtractor={(i) => String(i.tmdbId)}
            showsHorizontalScrollIndicator={false}
            contentContainerStyle={{ paddingHorizontal: 16, gap: 10 }}
            renderItem={({ item }) => (
              <TouchableOpacity onPress={() => nav.navigate("ShowDetail", { tmdbId: item.tmdbId })}>
                <View style={s.posterCard}>
                  ...
                  <Text style={s.posterLabel} numberOfLines={2}>{item.title}</Text>
                  {item.seasonNumber != null && item.episodeNumber != null && (
                    <Text style={s.posterMeta}>S{String(item.seasonNumber).padStart(2,"0")} E{String(item.episodeNumber).padStart(2,"0")}</Text>
                  )}
                </View>
              </TouchableOpacity>
            )}
        ...
        <Section title="Schedule">
          {schedule.slice(0, 8).map((item) => (
            <TouchableOpacity
              ...
              style={s.schedRow}
              onPress={() => nav.navigate("ShowDetail", { tmdbId: item.showTmdbId! })}
            >
              ...
              <View style={s.schedInfo}>
                <Text style={s.schedTitle} numberOfLines={1}>{item.showTitle}</Text>
      ...
      {(showRecs.length > 0 || movieRecs.length > 0) && (
        <>
          {showRecs.length > 0 && (
            <Section title="Recommended Shows">
              <FlatList
                horizontal
                data={showRecs}
                keyExtractor={(i) => String(i.tmdbId)}
          ...
          {movieRecs.length > 0 && (
            <Section title="Recommended Movies">
              <FlatList
                ...
                contentContainerStyle={{ paddingHorizontal: 16, gap: 10 }}
                renderItem={({ item }) => (
                  <TouchableOpacity onPress={() => nav.navigate("MovieDetail", { tmdbId: item.tmdbId })}>
                    <View style={s.posterCard}>
                      {item.posterPath ? (
                        <Image source={{ uri: `${TMDB_IMG}w185${item.posterPath}` }} style={s.posterImg} contentFit="cover" />
                      ) : (
...
function HeroSection({ alltime, heroArt }: { alltime: StatsAllTime | null; heroArt: string[] }) {
  return (
    <View style={s.hero}>
      {heroArt.length > 0 && (
        <Image
          source={{ uri: `${TMDB_IMG}w500${heroArt[Math.floor(Math.random() * Math.min(heroArt.length, 10))]}` }}
      ...
      <View style={s.heroContent}>
        <Text style={s.heroTitle}>TRAKT</Text>
        {alltime && (
...
const s = StyleSheet.create({
  ...
  heroTitle: { fontSize: 28, fontWeight: "900", color: "#e2e2e2", letterSpacing: 5, marginBottom: 12 },
  statsRow: { flexDirection: "row", gap: 20 },
  statChip: { alignItems: "center" },
  statValue: { fontSize: 22, fontWeight: "900", color: "#fff" },
  statLabel: { fontSize: 10, color: "rgba(255,255,255,0.5)", letterSpacing: 1, textTransform: "uppercase", marginTop: 2 },
  ...
  nowPlayingBadgeText: { fontSize: 10, fontWeight: "900", color: "rgba(255,255,255,0.6)", letterSpacing: 2 },
  ...
  progressPct: { fontSize: 11, fontWeight: "700", color: "#fff", textAlign: "right" },
  ...
  posterLabel: { fontSize: 11, color: "#cccccc", marginTop: 5, lineHeight: 14 },
  posterMeta: { fontSize: 10, color: "#888", marginTop: 2 },
