...
const mockGetDashboardHeroArt = vi.fn();
...
vi.mock("@/lib/api", () => ({
  api: {
    ...
    getDashboardHeroArt: (...args: unknown[]) => mockGetDashboardHeroArt(...args),
  },
}));
...
describe("DashboardPage", () => {
  ...
  it("renders up-next and schedule sections after data loads", async () => {
    ...
    mockGetDashboardHeroArt.mockResolvedValue([]);
  ...
  it("renders without crashing when all dashboard fetches fail", async () => {
    ...
    mockGetDashboardHeroArt.mockRejectedValue(new Error("Server error"));
